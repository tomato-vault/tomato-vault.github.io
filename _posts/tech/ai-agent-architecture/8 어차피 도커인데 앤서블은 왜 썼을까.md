...
나중에 알게된건데, 우리 서비스 정도 수준이면 로드밸런서는 전혀 필요가 없었다. 겨우 몇개 학원에서 사용하는, 많아야 수백명 정도 수준의 트래픽을 감당하는데 ALB라니... 또, MongoDB도 맹목적으로 좋다며 사용하라고 종용하셨었는데, 이 또한 비효율의 극치였다. MongoDB를 사용함에 있어서 매니지드 서비스를 이용하기 위해 결국 MongoDB Atlas를 이용했는데 이 덕분에 VPC Peering등 아키텍처적으로 복잡해진 것은 물론, 우리 서비스는 Document DB를 도입할 명분이 전혀 없었다. 이 경험은 대표의 기술적 이해도에 실망하게된 일화이기도하다.
{:.note title="여담"}

나는 2~3주간 AWS와 네트워크 관련 서적, 기술 블로그, 유튜브 강의, GPT와의 대화를 총동원해 밤낮없이 파고들었다. 구조를 머릿속에 넣기 위해 종이 위에 VPC 대역과 서브넷, ALB와 EC2 간의 패킷 흐름도를 직접 볼펜으로 그려가며 개념을 하나씩 메워나갔다.

하지만 개념을 익히고 콘솔 화면을 조작할 수 있게 되었음에도, 마우스로 인프라를 관리하는 행위는 비직관적이었고 개발자스럽지 못하다고 느꼈다.
...
```mermaid
flowchart TD
    classDef flow fill:#f8fafc,stroke:#64748b,stroke-width:1.5px,color:#0f172a
    classDef risk fill:#fff1f2,stroke:#f43f5e,stroke-width:1.5px,color:#881337

    subgraph CONSOLE ["<b>AWS 웹 콘솔 (수기 마우스 클릭 관리)</b>"]
        direction LR
        C1["<b>1. 네트워크</b><br/>VPC ➔ Subnet"]:::flow
        C2["<b>2. 라우팅 & 보안</b><br/>Route Table ➔ 보안 그룹 (SG)"]:::flow
        C3["<b>3. 컴퓨팅</b><br/>ALB ➔ EC2"]:::flow
        C1 --> C2 --> C3
    end

    CONSOLE ==>|"마우스 수기 조작의 한계"| RISKS

    RISKS["<div style='text-align:center;'><b>수동 클릭 운영의 4대 치명적 리스크</b></div><hr style='margin:8px 0; border:none; border-top:1px solid #fda4af;'/><div style='text-align:left; line-height:1.7;'>• <b>X 네트워크 단절 위험</b>: 클릭 실수 하나로 실서비스 트래픽 블랙홀 소실<br/>• <b>X 변경 이력(Audit Trail) 부재</b>: Git 커밋처럼 전후 변경 추적 불가<br/>• <b>X 환경 복제(Replication) 불가</b>: 스테이징 및 신규 테넌트 수기 재현 불가능<br/>• <b>X 즉각 롤백(Rollback) 불가</b>: 장애 발생 시 원래 정상 설정 복원 지연</div>"]:::risk

    style CONSOLE fill:#ffffff,stroke:#cbd5e1,stroke-width:1.5px
```

이대로 마우스 클릭에 의존하다가는, 언젠가 내 손으로 프로덕션을 날려 먹는 대형 사고를 치고 말 것만 같았다. 인프라 관리법에 대한 명확한 방법이 절실했다.

---

## 2. 콘솔에서 코드로: Terraform 도입과 인프라 경계 선언

마우스 클릭의 비효율과 공포에서 벗어나기 위해 GPT에게 실마리를 물었다. 돌아온 답은 명확했다. **<mark class="kw-term">Terraform</mark>**을 사용한 코드형 인프라(IaC)였다.

때마침 대표님이 2~3일간 외부 출장을 가게 되었다. 눈치를 보지 않고 새로운 기술을 온전히 실험해 볼 수 있는 절호의 기회였다. 나는 그 기간 동안 유튜브의 테라폼 강의와 공식 문서를 집요하게 파고들며 사내 AWS 계정에 적용할 테라폼 코드를 한 줄 한 줄 작성했다. 당시 가장 많이 참고하고 큰 길잡이가 되어 준 영상은 DevOps Directive의 [Complete Terraform Course - From BEGINNER to PRO! (Learn Infrastructure as Code)](https://youtu.be/7xngnjfIlK4)였다. 기본 문법부터 AWS 실전 리소스 프로비저닝까지 명쾌하게 짚어주는 흐름을 따라가며 인프라를 코드로 선언하는 감각을 온전히 익힐 수 있었다.

이때 테라폼을 공부하면서 접했던 여러 영상에서는 Terraform과 **<mark class="kw-term">Ansible</mark>**을 함께 조합했을 때의 강력한 시너지를 자주 강조하곤 했다. 인프라를 프로비저닝하는 테라폼과, 서버 내부 환경을 세팅하는 앤서블을 묶으면 배포 자동화의 완성도가 한층 높아진다는 설명이었다. 예컨대 [Terraform과 Ansible, CI/CD를 엮는 홈랩 자동화 강의](https://youtu.be/r5RckLwzkfc)나, 플레이북 기초를 직관적으로 풀어낸 [Ansible Playbook Basics 강의](https://youtu.be/p9bda0-TIRc), 그리고 플레이북 품질을 검증하는 [ansible-lint 워크스루 강의](https://youtu.be/8-M66pCzVTQ) 같은 영상들이 깊은 인상을 남겼다. 당시에는 '이런 기술 조합도 있구나' 하고 머릿속에 담아두었을 뿐이지만, 이 기억이 훗날 인스턴스 초기화 난관을 마주했을 때 Ansible을 주저 없이 도입하게 된 결정적 계기가 되었다.
{:.note title="여담: Terraform 학습 중 접한 Ansible의 씨앗"}
...
무엇보다 IaC로의 전환은 **AI 에이전트와의 협업 환경에서도 압도적인 위력**을 발휘했다. 마우스 클릭 기반의 콘솔 운영은 AI에게 공유하거나 위임할 수 없는 '인간만의 암묵지'이자 블랙박스 영역이다. AI에게 웹 콘솔의 수많은 메뉴와 버튼을 대신 클릭해 달라고 할 수는 없기 때문이다(물론 요새는 Chrome 등 브라우저에서 에이전트를 띄워서 적절히 활용할 수 있겠지만 그 때 당시만하더라도 그런 기능은 없었다). 반면 인프라가 텍스트 코드로 정의되는 순간, 인프라는 저장소 안에서 Git의 추적을 받는 일급 시민(First-class citizen)이 된다.

인간과 AI 에이전트가 완벽하게 동일한 단일 진실 공급원(SSOT)을 공유하게 된 것이다. 새로운 서브넷 구성이나 보안 그룹 규칙 추가, ALB 리스너 라우팅 변경이 필요할 때 에이전트는 기존 테라폼 모듈의 구조를 한눈에 파악하고 정확한 HCL 코드를 제안할 수 있었다. 게다가 에이전트가 작성한 코드가 안전한지는 사람이 눈대중으로 감각에 의존할 필요 없이, `terraform plan`이 뱉어내는 결정론적 diff를 에이전트와 함께 교차 검증하면 그만이었다. 리소스가 의도치 않게 삭제되거나(destroy) 재생성(replace)되는 위험을 사전에 완벽히 차단할 수 있었다. 결국 IaC는 혼자서 인프라를 관리하는 개발자에게 단순한 자동화 도구를 넘어, 인프라 작업 전체를 AI와 고도화된 페어로 수행할 수 있게 만들어 준 든든한 기술적 디딤돌이었다.
...
```mermaid
flowchart TD
    classDef pastFlow fill:#fff1f2,stroke:#f43f5e,stroke-width:1.5px,color:#881337
    classDef presentFlow fill:#f0fdf4,stroke:#22c55e,stroke-width:1.5px,color:#14532d
    classDef decision fill:#fffbeb,stroke:#f59e0b,stroke-width:2px,color:#78350f
    classDef card fill:#ffffff,stroke:#cbd5e1,stroke-width:1.5px,color:#1e293b

    subgraph PAST ["<b>과거: Ansible 기반 호스트 형상 관리 (가변 인프라)</b>"]
        direction LR
        P1["<b>1. EC2 기동</b><br/>호스트 생성"]:::pastFlow
        P2["<b>2. SSH 접속</b><br/>원격 포트 연결"]:::pastFlow
        P3["<b>3. Ansible 실행</b><br/>호스트 패키지 설치"]:::pastFlow
        P4["<b>4. Docker 기동</b><br/>호스트 소스 빌드"]:::pastFlow
        P1 --> P2 --> P3 --> P4
    end

    P_RISK["<div style='width:540px; text-align:left; line-height:1.7;'><b>X 과거의 한계 (운영 오버헤드)</b><br/>• <b>호스트 환경 오염(Configuration Drift)</b>: 인스턴스마다 OS 패키지 상태 불일치<br/>• <b>보안 복잡성</b>: SSH 포트 상시 오픈 및 EC2 접속 키 관리 부담<br/>• <b>배포 지연</b>: 매 배포마다 호스트 패키지 재설치로 10분 이상 소요</div>"]:::card

    PAST ==> P_RISK
    P_RISK ==>|"운영 오버헤드 ➔ 단순화 결단"| DECISION

    DECISION["<div style='width:540px; text-align:center; padding:6px 0; line-height:1.7;'><b>단순화 결단: Ansible 전면 폐기</b><br/><span style='color:#b45309;'>'로컬도 Docker, 서버도 Docker인데 왜 호스트 OS를 관리하는가?'</span><br/><b>호스트 OS 관리 완전 위임 ➔ 불변 컨테이너 단일 아티팩트 전환</b></div>"]:::decision

    DECISION ==>|"불변 아티팩트 배포 체계"| PRESENT

    subgraph PRESENT ["<b>현재: Docker + ECR · ECS 파이프라인 (불변 인프라)</b>"]
        direction LR
        C1["<b>1. Docker Build</b><br/>불변 이미지 빌드"]:::presentFlow
        C2["<b>2. ECR Push</b><br/>고유 태그 이미지"]:::presentFlow
        C3["<b>3. Task Def</b><br/>새 리비전 등록"]:::presentFlow
        C4["<b>4. ECS 배포</b><br/>롤링 업데이트"]:::presentFlow
        C1 --> C2 --> C3 --> C4
    end

    PRESENT ==> C_BENEFIT

    C_BENEFIT["<div style='width:540px; text-align:left; line-height:1.7;'><b>O 현재의 이점 (불변 인프라)</b><br/>• <b>호스트 OS 무관심</b>: AWS Fargate / ECS AMI에 OS 관리 완전 위임<br/>• <b>보안 격리</b>: 호스트 SSH 포트 완전 차단, 컨테이너 단위 격리<br/>• <b>초고속 배포 및 즉각 롤백</b>: 2~3분 내 롤링 배포 및 이전 태스크 리비전 즉시 복원</div>"]:::card

    style PAST fill:#ffffff,stroke:#fda4af,stroke-width:1.5px
    style PRESENT fill:#ffffff,stroke:#86efac,stroke-width:1.5px
```
